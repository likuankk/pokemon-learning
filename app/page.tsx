...
export default function HomePage() {
  ...
  if (showPokemonSelect) {
    return (
      <div className="flex flex-col md:flex-row h-screen bg-gradient-to-br from-blue-500 via-purple-500 to-pink-400">
        {/* Left: preview - hidden on mobile, or smaller */}
        <div className="hidden md:flex w-full md:w-1/2 flex-col items-center justify-center p-10">
        ...
        {/* Right: grid */}
        <div className="w-full md:w-1/2 bg-white flex flex-col justify-center px-6 py-8 md:px-10 overflow-y-auto">
          {/* Mobile header */}
          <div className="md:hidden text-center mb-6">
            <div className="text-6xl mb-3">🌟</div>
            <h1 className="game-title-indigo" style={{ fontSize: '2rem', color: '#4f46e5' }}>选择你的伙伴！</h1>
          </div>
          <h2
            className="hidden md:block game-title-indigo mb-8"
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5 mb-8">
            {STARTER_POKEMON.map((p) => (
              <motion.button
                ...
                disabled={loading}
                className="flex flex-col items-center p-3 md:p-5 rounded-3xl border-3 transition-all game-card"
                style={{
              ...
              >
                <img
                  ...
                  width={130}
                  height={130}
                  className="w-20 h-20 md:w-[130px] md:h-[130px]"
                  style={{
                    objectFit: 'contain',
                    filter: 'drop-shadow(0 8px 10px rgba(0,0,0,0.25))',
                  }}
                />
                <span className="game-label text-base md:text-xl mt-2 font-bold">{p.name}</span>
                <span className="text-sm md:text-base text-gray-400 mt-1 font-bold" style={{ fontFamily: "'ZCOOL KuaiLe', sans-serif" }}>{p.type}</span>
              </motion.button>
            ))}
          </div>
          <button
            onClick={() => setShowPokemonSelect(false)}
            className="text-gray-400 hover:text-gray-600 text-xl md:text-2xl font-bold text-left"
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen overflow-hidden">
      {/* Left: hero */}
      <div className="w-full md:w-3/5 bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 flex flex-col items-center justify-center relative overflow-hidden py-10 md:py-0">
        ...
        >
          ...
          <p
            className="text-white/80 mb-8 md:mb-14"
            style={{ fontFamily: "'ZCOOL KuaiLe', sans-serif", fontSize: 'clamp(1.25rem, 3vw, 2.25rem)' }}
          >
            学习让伙伴更强大！
          </p>
            ...
            {DISPLAY_POKEMON.slice(0, 3).map((id, i) => (
              <motion.div key={id} className="pokemon-3d-container">
                <motion.img
                  ...
                  width={150}
                  height={150}
                  className="w-16 h-16 md:w-[150px] md:h-[150px]"
                  style={{
                    objectFit: 'contain',
            ...
            {DISPLAY_POKEMON.slice(3, 6).map((id, i) => (
              <motion.div key={id} className="pokemon-3d-container">
                <motion.img
                  ...
                  width={120}
                  height={120}
                  className="w-12 h-12 md:w-[120px] md:h-[120px]"
                  style={{
                    objectFit: 'contain',
        ...
        <motion.p
          className="absolute bottom-4 md:bottom-8 text-white/40 text-sm md:text-xl"
          style={{ fontFamily: "'ZCOOL KuaiLe', sans-serif" }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
      ...
      {/* Right: entry */}
      <div className="w-full md:w-2/5 bg-white flex flex-col items-center justify-center px-6 py-8 md:px-16">
        <motion.div
        ...
        >
          ...
          <p
            className="text-gray-400 mb-8 md:mb-12"
            style={{ fontFamily: "'ZCOOL KuaiLe', sans-serif", fontSize: 'clamp(1.1rem, 2vw, 1.75rem)' }}
          >
            请选择你的身份进入
          </p>
          ...
          <div className="space-y-4 md:space-y-6">
            <motion.button
              ...
              className="w-full bg-indigo-50 hover:bg-indigo-100 rounded-3xl p-5 md:p-8 flex items-center gap-4 md:gap-6 transition-all group game-card"
              style={{
            ...
            >
              ...
              <div className="text-left flex-1">
                ...
                <div
                  ...
                  style={{ fontFamily: "'ZCOOL KuaiLe', sans-serif", fontSize: 'clamp(0.9rem, 1.5vw, 1.5rem)' }}
                >
                  出题 · 审核 · 守护成长
                </div>
              </div>
              <span className="text-gray-400 group-hover:text-indigo-500 text-3xl md:text-5xl">›</span>
            </motion.button>

            <motion.button
              onClick={() => setShowPokemonSelect(true)}
              className="w-full bg-yellow-50 hover:bg-yellow-100 rounded-3xl p-5 md:p-8 flex items-center gap-4 md:gap-6 transition-all group game-card"
              style={{
            ...
            >
              ...
              <span className="text-gray-400 group-hover:text-yellow-600 text-3xl md:text-5xl">›</span>
            </motion.button>
          </div>
        </motion.div>
      </div>
